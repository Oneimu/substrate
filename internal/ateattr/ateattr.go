...
// the ateom_restore / ateom_checkpoint phases above, promoted from ad-hoc log
// fields into the shared phase vocabulary. Unlike the atelet phases, the
// restore phases here are sequential and partition the total; the checkpoint
// captures (snapshot, durable_dir, rootfs_upper) run concurrently on the
// paused guest, so those three are independent observations.
const (
	// Checkpoint phases.
	...
	SnapshotPhaseWakeupProbe = "wakeup_probe"
)
