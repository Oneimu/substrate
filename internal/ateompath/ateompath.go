...
// overlay uppers (ateom-microvm): one subdirectory per container, served into
// the guest over virtio-fs so rootfs writes land on host disk instead of guest
// RAM. Unlike the durable-dir volumes it is owned entirely by ateom (created at
// cold boot, archived at checkpoint, removed at teardown); atelet never touches
// it.
