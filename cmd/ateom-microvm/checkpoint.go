...
func (s *AteomService) CheckpointWorkload(ctx context.Context, req *ateompb.CheckpointWorkloadRequest) (*ateompb.CheckpointWorkloadResponse, error) {
	...
	var extra []slog.Attr
	if snapStats.merged {
		extra = append(extra, slog.Int64(deltaSizeKey, snapStats.deltaBytes))
	}
	logSnapshotPhases(ctx, "Checkpoint timing breakdown", attribution, scope,
		checkpointDurationKey, []phase{
			{ateattr.SnapshotPhasePause, dPause},
			{ateattr.SnapshotPhaseSnapshot, snapStats.snapshot},
			{ateattr.SnapshotPhaseDurableDir, dDurable},
			{ateattr.SnapshotPhaseRootfsUpper, dUpper},
			{ateattr.SnapshotPhaseMerge, snapStats.merge},
			{ateattr.SnapshotPhaseTeardown, dTeardown},
